项目简历：
基于YOLOv8实现的交通场景目标检测桌面应用程序。
本人项目角色：AI模型数据与评测；主要负责数据集清洗预处理、样本标注校验、AI模型多维度评测分析、系统端到端测试与项目文档归档。
软件支持图片、本地视频、摄像头实时画面检测，具备参数调节、结果可视化、批量推理、结果导出保存完整能力，针对小目标、物体遮挡场景做模型调优，输出完整模型评估报告。
